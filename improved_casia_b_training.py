"""
Improved CASIA-B Training for High Accuracy
Uses more data and advanced features to achieve 90%+ accuracy
"""
...
class ImprovedCASIABLoader:
    """
    Improved loader for CASIA-B dataset with advanced features
    """
    ...
    def load_gait_sequences(self, max_subjects: int = None, max_sequences_per_subject: int = 5):
        """
        Load more gait sequences for better accuracy
        """
    ...
    def _load_sequence_from_directory(self, directory_path: str) -> np.ndarray:
        """Load a gait sequence from directory"""
        try:
    ...
    def extract_advanced_gait_features(self, sequence: np.ndarray) -> dict:
        """
        Extract advanced gait features for high accuracy
        """
        features = {}
        ...
        # Advanced gait cycle analysis
        gait_features = self._analyze_advanced_gait_cycle(sequence)
        features.update(gait_features)
        
        # Enhanced shape features
        shape_features = self._extract_enhanced_shape_features(sequence)
    ...
    def _analyze_advanced_gait_cycle(self, sequence: np.ndarray) -> dict:
        """Analyze advanced gait cycle characteristics"""
        features = {}
        ...
        if len(silhouette_areas) > 1:
            ...
            features.update({
                ...
                'gait_cycle_period': len(sequence) / (len(peaks) + 1e-8),
                'gait_cycle_amplitude': np.std(silhouette_areas),
            })
        else:
            features.update({
                'gait_cycle_peaks': 0, 'gait_cycle_valleys': 0, 'gait_regularity': 0,
                'gait_cycle_frequency': 0, 'gait_symmetry': 0, 'gait_rhythm': 0,
                'gait_cycle_period': 0, 'gait_cycle_amplitude': 0,
            })
        
        return features
    
    def _extract_enhanced_shape_features(self, sequence: np.ndarray) -> dict:
        """Extract enhanced shape and proportion features"""
        features = {}
    ...
    def _extract_temporal_dynamics(self, sequence: np.ndarray) -> dict:
        ...
        if frame_diffs:
            features.update({
                ...
                'motion_intensity': np.sum(frame_diffs),
                'motion_consistency': 1 / (1 + np.std(frame_diffs) / (np.mean(frame_diffs) + 1e-8)),
            })
        else:
            features.update({
                'avg_frame_difference': 0, 'frame_difference_std': 0,
                'temporal_smoothness': 0, 'motion_intensity': 0, 'motion_consistency': 0,
            })
        
        return features
    
    def _extract_gei_features(self, sequence: np.ndarray) -> dict:
        """Extract Gait Energy Image (GEI) features"""
        features = {}
        
        if len(sequence) > 0:
            # Create Gait Energy Image (average across frames)
            gei = np.mean(sequence, axis=0)
            
            # GEI statistical features
            features['gei_mean'] = np.mean(gei)
            features['gei_std'] = np.std(gei)
            features['gei_energy'] = np.sum(gei**2)
            features['gei_variance'] = np.var(gei)
            
            # GEI spatial features
            if np.sum(gei) > 0:
            ...
            # GEI texture features
            features['gei_contrast'] = np.std(gei)
            features['gei_homogeneity'] = 1 / (1 + np.std(gei))
        
        return features
    
    def _extract_frequency_features(self, sequence: np.ndarray) -> dict:
        """Extract frequency domain features"""
        features = {}
        
        if len(sequence) > 1:
            # Calculate silhouette area over time
            areas = [np.sum(frame > 0) for frame in sequence]
            
            # FFT analysis
            fft_areas = np.fft.fft(areas)
            magnitude = np.abs(fft_areas)
            
            # Frequency features
            if len(magnitude) > 2:
                features['dominant_frequency'] = np.argmax(magnitude[1:len(magnitude)//2]) + 1 if len(magnitude[1:len(magnitude)//2]) > 0 else 0
            else:
            ...
            if np.sum(magnitude) > 0:
                ...
                features['spectral_bandwidth'] = np.sqrt(np.sum((np.arange(len(magnitude)) - features['spectral_centroid'])**2 * magnitude**2) / np.sum(magnitude**2))
            else:
                features['spectral_centroid'] = 0
                features['spectral_bandwidth'] = 0
        else:
            features.update({
                'dominant_frequency': 0, 'spectral_energy': 0,
    ...
    def _get_default_features(self) -> dict:
        ...
        # Basic features
        basic_features = ['sequence_length', 'avg_silhouette_area', 'silhouette_area_std', 'silhouette_area_range']
        for feature in basic_features:
            default_features[feature] = 0
        
        # Centroid features
        centroid_features = ['center_of_mass_variation_x', 'center_of_mass_variation_y', 'center_of_mass_range_x', 
                           'center_of_mass_range_y', 'center_of_mass_velocity_x', 'center_of_mass_velocity_y',
                           'center_of_mass_acceleration_x', 'center_of_mass_acceleration_y']
        for feature in centroid_features:
            default_features[feature] = 0
        
        # Gait cycle features
        gait_features = ['gait_cycle_peaks', 'gait_cycle_valleys', 'gait_regularity', 'gait_cycle_frequency',
                        'gait_symmetry', 'gait_rhythm', 'gait_cycle_period', 'gait_cycle_amplitude']
        for feature in gait_features:
            default_features[feature] = 0
        
        # Shape features
        shape_features = ['avg_width', 'avg_height', 'width_variation', 'height_variation',
                         'avg_aspect_ratio', 'aspect_ratio_variation', 'width_height_ratio', 'body_proportion']
        for feature in shape_features:
            default_features[feature] = 0
        
        # Temporal features
        temporal_features = ['avg_frame_difference', 'frame_difference_std', 'temporal_smoothness',
                           'motion_intensity', 'motion_consistency']
        for feature in temporal_features:
            default_features[feature] = 0
        
        # GEI features
        gei_features = ['gei_mean', 'gei_std', 'gei_energy', 'gei_variance', 'gei_centroid_x', 'gei_centroid_y',
                       'gei_contrast', 'gei_homogeneity']
        for feature in gei_features:
            default_features[feature] = 0
        
        # Frequency features
        freq_features = ['dominant_frequency', 'spectral_energy', 'spectral_centroid', 'spectral_bandwidth']
    ...
    def create_feature_dataset(self) -> tuple:
        ...
        print("🔍 Extracting advanced gait features...")
...
class HighAccuracyClassifier:
    """High-accuracy classifier with ensemble methods"""
    ...
    def initialize_models(self):
        ...
        self.models = {
            ...
            'Neural Network': MLPClassifier(
                hidden_layer_sizes=(300, 150, 75), activation='relu',
                solver='adam', alpha=0.0001, max_iter=2000,
                random_state=self.random_state, early_stopping=True
            ),
            'Logistic Regression': LogisticRegression(
                C=10.0, max_iter=3000, random_state=self.random_state,
    ...
    def create_ensemble(self, X_train, y_train):
        ...
        for name, model in self.models.items():
            try:
                ...
                if scores.mean() > 0.3:  # Include models with >30% CV score
...
def run_improved_training(dataset_path: str):
    """Run improved CASIA-B training"""
    print("🎯 IMPROVED CASIA-B GAIT RECOGNITION TRAINING")
    print("=" * 60)
    ...
    processor = ImprovedCASIABLoader(dataset_path)
    
    # Load more sequences for better accuracy
    total_sequences = processor.load_gait_sequences(max_subjects=50, max_sequences_per_subject=4)
    ...
    if best_test_score >= 0.90:
        ...
        print("💡 Good progress - consider more data or model tuning")
    else:
...
if __name__ == "__main__":
    ...
    classifier, results, cv_results = run_improved_training(dataset_path)
